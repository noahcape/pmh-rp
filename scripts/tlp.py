...
class MultiLabeledTree:
    ...
    def pull_down(self, out=None):
        """
        Pull down all nodes assigning the correct labeling to
        those pulled down leaves and removing the label of the parent node.
        """
    ...
    def solve_tlp(self, l):
        ...
        model = create_tree_labeling_polytope(
            self.tree, root, self.character_set, self.leaf_f, self.dist_f
        )
        model = set_regularized_parsimony_objective(
            self.tree, self.character_set, model, self.dist_f, l
        )
...
def solve_pareto_front(
    ...
    solver = pyo.SolverFactory("gurobi_persistent")
    model = create_tree_labeling_polytope(
        tree, root, character_set, leaf_f, dist_f, root_label
    )
    model = set_regularized_parsimony_objective(
        tree, character_set, model, dist_f, weights[0]
    )
    ...
    if len(weights) > 1:
        ...
        for lambda_ in weights[1:]:
            ...
            # set a new objective
            model = set_regularized_parsimony_objective(
                tree, character_set, model, dist_f, lambda_, False
            )
            solver.set_instance(model)
        ...
        try:
            (knee_x, knee_y) = find_knee(
                x_regularization_weights, y_parsimony_score, 0.5, "concave"
            )
            if knee_x == None or knee_y == None:
                (knee_x, knee_y) = find_knee(
                    x_regularization_weights, y_parsimony_score, 0.5, "convex"
                )
        ...
        write_results(
            migration_graph, vertex_labeling, output, {"regularizer_weight": knee_x}
        )
    else:
        # if only one weight just solve and continue
        (migration_graph, vertex_labeling) = solutions[weights[0]]
        write_results(
            migration_graph, vertex_labeling, output, {"regularizer_weight": weights[0]}
        )
...
def set_regularized_parsimony_objective(
...
def setup_fast_machina_constraints(solver, model, character_set, constraint_type):
    def dag_callback(model, gb_model, where):
        ...
        # load solutions
        gb_model.cbGetSolution(
            [
                model.migrations[i, j]
        ...
        try:
            ...
            cons = model.migration_graph_constraints.add(
                sum(
                    model.migrations[(S[i], S[(i + 1) % len(S)])] for i in range(len(S))
                )
                <= len(S) - 1
            )
...
def campelo_et_al(tree, character_set, leaf_f, dist_f, root, args, integral=True):
    ...
    if integral:
        ...
        model.x = pyo.Var(
            T.nodes, character_set, domain=pyo.NonNegativeReals, bounds=(0, 1)
        )

    logger.info(
        f"Creating Campelo et al. 2016 formulation for tree with {len(T.nodes)} nodes and {len(T.edges)} edges."
    )
            ...
            for w in path[1:-1]:
                for c in character_set:
                    model.path_constraints.add(
                        model.x[u, c] + model.x[v, c] - model.x[w, c] <= 1
                    )

    for u in T.nodes:
        model.path_constraints.add(sum(model.x[u, c] for c in character_set) == 1)

    def weight_f(u, c):
        if rooted_T.out_degree(u) != 0:
            return 0
        if c == leaf_f(u):
            return 0
        return 1
    ...
    model.objective_expr = sum(
        weight_f(u, c) * model.x[u, c] for u in T.nodes for c in character_set
    )
    model.objective = pyo.Objective(expr=model.objective_expr, sense=pyo.minimize)
...
def parsimonious_relabeling(
    tree, character_set, leaf_f, dist_f, root, args, integral=True
):
    T = tree.copy()
    ...
    if integral:
        model.decisions = pyo.Var(
            T.edges, character_set, character_set, domain=pyo.Binary, initialize=0
        )
        ...
        model.relabelings = pyo.Var(
            pendant_edges, domain=pyo.NonNegativeReals, bounds=(0, 1)
        )
    ...
    for u, v in pendant_edges:
        for c in character_set:
            if c == leaf_f(v):
                model.leaf_constraints.add(
                    sum(model.decisions[u, v, c2, c] for c2 in character_set)
                    == model.relabelings[u, v]
                )

    # c^T x_{u,v,i,j} \leq k
    model.sum_constraints.add(
        sum(
            model.decisions[u, v, c1, c2] * dist_f((u, v), c1, c2)
            for u, v in T.edges
            for c1 in character_set
            for c2 in character_set
        )
...
def fast_machina(tree, character_set, leaf_f, dist_f, root, args, mip_gap=0.15):
    ...
    if args.constraints.startswith("monoclonal"):
        for c1, c2 in model.migrations:
            model.migration_constraints.add(
                sum(model.decisions[u, v, c1, c2] for u, v in tree.edges)
                <= model.migrations[c1, c2]
            )
...
def parse_arguments():
    ...
    fast_machina_parser.add_argument(
        "-c",
        "--constraints",
        help="Migration graph constraints",
        choices=[
            "polyclonal_tree",
            "polyclonal_dag",
            "monoclonal_tree",
    ...
    fast_machina_parser.add_argument(
        "-r", "--root", help="Root of the tree", default=None
    )
    fast_machina_parser.add_argument("-l", "--label", help="Root label", default=None)
    fast_machina_parser.add_argument(
        "-e",
        "--edge-regularized",
        help="Regularize normal parsimony with number of edges in migraton graph",
        type=int,
        default=0,
    )
    fast_machina_parser.add_argument(
        "-w",
        "--regularizer-weight",
        help="How much to weigh edge regularization",
        type=float,
        default=1.0,
    )
    fast_machina_parser.add_argument(
        "-n",
        "--clone-tree-instance",
        help="Expect a clone tree with internal and multilabels",
        type=int,
        default=0,
    )

    # parsimonious relabeling subparser
    parsimonious_relabeling = subparsers.add_parser(
        "convex_recoloring", help="parsimoniousRelabeling"
    )
    ...
    parsimonious_relabeling.add_argument(
        "-o", "--output", help="Output prefix", default="result"
    )
    parsimonious_relabeling.add_argument(
    ...
    parsimonious_relabeling.add_argument(
        "-k", help="Weighted parsimony constraint", default=None, type=float
    )
    parsimonious_relabeling.add_argument(
        "-w", "--weights", help="Weight of transitioning between labels", default=None
    )
    parsimonious_relabeling.add_argument(
        "-m", "--mode", help="Mode", choices=["campelo", "tlp"], default="tlp"
    )
...
if __name__ == "__main__":
    ...
    if clone_tree_instance:
        ...
        if not all(
            [
                clone_tree.label_f(node) is not None
                for node in clone_tree.tree.nodes
                if is_leaf(clone_tree.tree, node)
            ]
        ):
            unlabeled_leaves = [
                node
                for node in clone_tree.tree.nodes
                if is_leaf(clone_tree.tree, node) and clone_tree.label_f(node) is None
            ]
        ...
        weights = [1]
        root = [v for v in clone_tree.tree.nodes if clone_tree.tree.in_degree(v) == 0][
            0
        ]

        mip_gap = 0
        # 0.05 if len(clone_tree.tree.nodes) < 25 else 0.15
        ...
        if not all(
            ...
            unlabeled_leaves = [
                ...
                if is_leaf(tree, node) and leaf_f(node) is None
            ]
            raise ValueError(f"Leaves {unlabeled_leaves} are unlabeled.")
        ...
        if args.edge_regularized == 0:
            if len(character_set) == 1:
                ...
                if args.method == "fast_machina":
                    ...
                    if args.mode == "tlp":
                        _, lp_obj = parsimonious_relabeling(
                            tree,
                            character_set,
                            leaf_f,
                            dist_f,
                        ...
                        vertex_labeling, obj = parsimonious_relabeling(
                            tree, character_set, leaf_f, dist_f, root, args
                        )
                    else:
                        _, lp_obj = campelo_et_al(
                            tree,
                            character_set,
                            leaf_f,
                            dist_f,
                        ...
                        vertex_labeling, obj = campelo_et_al(
                            tree, character_set, leaf_f, dist_f, root, args
                        )
            ...
            weights = [1]
            mip_gap = 0.00
            # if len(tree.nodes) < 25 else 0.15
            solve_pareto_front(
                tree,
                root,
                character_set,
